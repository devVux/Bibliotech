...
public:
    MusicAlbulm(const QString& title,const QString& publisher,uint32_t year, const QString& artist, const QString& genre, uint32_t trackCount)
        : Media(title, publisher, year), mArtist(artist),  mGenre(genre), mTrackCount(trackCount) { }
